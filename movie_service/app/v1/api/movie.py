...
from movie_service.app.v1.core.deps import SessionDep
from movie_service.app.v1.model.movie import Movie
from movie_service.app.v1.schema.response import MoviePublic as MovieResponse, CustomResponse
from movie_service.app.v1.schema.request import MovieTitle
from movie_service.app.v1.api.crud import fetch_movies, remove_movie, create_movie as create_new_movie, update_movie as update_new_movie
...
async def get_movies(*, session: SessionDep, movie_title: MovieTitle, limit: int = 10, offset: int = 0, sort_by: str = None):
...
async def update_movie(*, session: SessionDep, movie_create: MovieCreate, movie_id: uuid.UUID):
    new_movie = update_new_movie(session=session, movie_id=movie_id, new_movie=movie_create)
    results = [new_movie]
    return CustomResponse(
                status_code=status.HTTP_200_OK,
                status="Success",
